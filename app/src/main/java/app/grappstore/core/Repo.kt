...
import app.grappstore.BuildConfig
import app.grappstore.PackageStates
import app.grappstore.R
import app.grappstore.util.asStringList
import app.grappstore.util.checkMainThread
import app.grappstore.util.getPackageInfoOrNull
import app.grappstore.util.isEven
import app.grappstore.util.maybeGetSystemFeatureInfo
...
enum class PackageSource(@param:StringRes val uiName: Int) {
    Grappstore(R.string.pkg_source_grappstore),
                        ...
                        json: JSONObject)
{
    ...
    val iconUrl: String? = run {
        val explicitUrl = json.opt("iconUrl") as String?
        if (explicitUrl != null) {
            return@run explicitUrl
        }
...
class Apk(
    ...
    fun downloadUrl() = downloadUrlOverride
        ?: "$REPO_BASE_URL/packages/${pkg.manifestPackageName}/${pkg.versionCode}/$name.gz"
